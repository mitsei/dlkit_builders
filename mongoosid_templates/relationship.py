...
class RelationshipLookupSession:
    ...
    get_relationships_for_source_template = """
        ...
        from .objects import ${object_name}List
        ...
        mongo_client.close()
        return ${object_name}List(result, count=count, runtime=self._runtime)"""
    ...
    get_relationships_by_genus_type_for_source_template = """
        ...
        if self._catalog_view == ISOLATED:
            ...
            count = collection.find({'${source_name_mixed}Id': str(source_id),
    ...
    get_relationships_for_destination_template = """
        ...
        if self._catalog_view == ISOLATED:
            ...
            result = collection.find({'destinationId': str(destination_id)}).sort('_sort_id', ASCENDING)
            count = collection.find({'destinationId': str(destination_id)}).count()
        mongo_client.close()
        return ${object_name}List(result, count=count, runtime=self._runtime)"""
...
class RelationshipAdminSession:
    ...
    get_relationship_form_for_create_template = """
        ...
        if ${arg2_name} == []:
            ...
            obj_form = ${return_type}(${cat_name_under}_id = self._catalog_id, ${arg0_name} = ${arg0_name}, ${arg1_name} = ${arg1_name}, catalog_id = self._catalog_id, db_prefix=self._db_prefix, runtime=self._runtime)
        else:
            obj_form = ${return_type}(${cat_name_under}_id = self._catalog_id, record_types = ${arg2_name}, ${arg0_name} = ${arg0_name}, ${arg1_name} = ${arg1_name}, catalog_id = self._catalog_id, db_prefix=self._db_prefix, runtime=self._runtime)
