...
def make_mgr_util_osid(file_name):
    ...
    if package['name'] not in packages_to_implement:
        return
    print("--> Manager Util Implement Package: {0}".format(package['name']))
...
def make_persistance_initers(persisted_data, initialized_data, aggregate_data):
    ...
    for data_name in persisted_data:
        if ((persisted_data[data_name] == 'osid.id.Id' or
            ...
            initers += '        self._my_map[\'' + under_to_mixed(data_name) + 'Id\'] = str(kwargs[\'' + data_name + '_id\'])\n'
        elif (persisted_data[data_name] == 'osid.resource.Resource' and
                data_name in initialized_data):
            initers += '        self._my_map[\'' + under_to_mixed(data_name) + 'Id\'] = str(kwargs[\'effective_agent_id\'])\n'
        elif persisted_data[data_name] == 'osid.id.Id':
            initers += ('        self._my_map[\'' + under_to_mixed(data_name) + 'Id\'] = self._' +
                        data_name + '_default\n')
        elif persisted_data[data_name] == 'osid.id.Id[]':
            initers += ('        self._my_map[\'' + under_to_mixed(remove_plural(data_name)) + 'Ids\'] = self._' +
                        data_name + '_default\n')
            ...
            initers += ('        self._my_map[\'' + under_to_mixed(data_name) + '\'] = self._' +
                        data_name + '_default\n')
    ...
    for data_name in aggregate_data:
        if aggregate_data[data_name].endswith('List'):
            initers += '        self._my_map[\'' + under_to_mixed(data_name) + '\'] = []\n'
        else:
            initers += '        self._my_map[\'' + under_to_mixed(data_name) + '\'] = None\n'
    ...
    for data_name in initialized_data:
        if data_name in persisted_data:
            ...
            initers += '        self._my_map[\'' + under_to_mixed(data_name) + 'Id\'] = \'\'\n'
        elif initialized_data[data_name] == 'osid.id.Id[]':
            initers += '        self._my_map[\'' + under_to_mixed(data_name) + 'Id\'] = []\n'
...
def make_method_impl(package_name, method, interface, patterns):
    ...
    # method implementation that serves as the pattern, if one exixts.
        ...
        if 'object_name' in kwargs and 'object_name_under' not in kwargs and 'object_name_upper' not in kwargs:
