"""TestAuthZ templates for assessment_authoring interfaces"""
    ...
    if not is_never_authz(request.cls.service_config):
        ...
        for ${cat_name_under}_ in request.cls.${cat_name_under}_list:
            ...
            for color in ['Red', 'Blue', 'Red']:
                create_form = ${cat_name_under}_.get_${object_name_under}_form_for_create_for_assessment(request.cls.assessment.ident, [])
                ...
                ${object_name_under} = ${cat_name_under}_.create_${object_name_under}_for_assessment(create_form)
...
def authz_adapter_test_fixture(request):
    ...
    if not is_never_authz(request.cls.service_config):
        create_form = request.cls.bank_list[0].get_assessment_form_for_create([])
        create_form.display_name = 'Assessment for SequenceRule Tests'
        create_form.description = 'Assessment for authz adapter tests for SequenceRule'
        request.cls.assessment = request.cls.bank_list[0].create_assessment(create_form)
        create_form = request.cls.bank_list[0].get_assessment_part_form_for_create_for_assessment(request.cls.assessment.ident, [])
        create_form.display_name = 'AssessmentPart for SequenceRule Tests'
        create_form.description = 'AssessmentPart for authz adapter tests for SequenceRule'
        request.cls.assessment_part = request.cls.bank_list[0].create_assessment_part_for_assessment(create_form)
        for ${cat_name_under}_ in request.cls.${cat_name_under}_list:
            request.cls.${object_name_under}_id_lists.append([])
            for color in ['Red', 'Blue', 'Red']:
                create_form = ${cat_name_under}_.get_${object_name_under}_form_for_create(
                    request.cls.assessment_part.ident, request.cls.assessment_part.ident, [])
                create_form.display_name = color + ' ' + str(count) + ' ${object_name}'
                create_form.description = color + ' ${object_name_under} for authz adapter tests from ${cat_name} number ' + str(count)
                if color == 'Blue':
                    create_form.genus_type = BLUE_TYPE
    ...
    def test_tear_down():
        if not is_never_authz(request.cls.service_config):
            for index, ${cat_name_under}_ in enumerate(request.cls.${cat_name_under}_list):
                for ${object_name_under}_id in request.cls.${object_name_under}_id_lists[index]:
                    ${cat_name_under}_.delete_${object_name_under}(${object_name_under}_id)
            request.cls.bank_list[0].delete_assessment_part(request.cls.assessment_part.ident)
            request.cls.bank_list[0].delete_assessment(request.cls.assessment.ident)
