...
def authz_adapter_class_fixture(request):
    ...
    if not is_never_authz(request.cls.service_config):
        ...
        request.cls.${pkg_name_replaced}_mgr = Runtime().get_service_manager(
            '${pkg_name_replaced_upper}',
            proxy=PROXY,
            implementation='TEST_SERVICE')
        for num in [0, 1, 2, 3, 4, 5, 6, 7]:
            create_form = request.cls.${pkg_name_replaced}_mgr.get_${cat_name_under}_form_for_create([])
            ...
            ${cat_name_under} = request.cls.${pkg_name_replaced}_mgr.create_${cat_name_under}(create_form)
            request.cls.${cat_name_under}_list.append(${cat_name_under})
            request.cls.${cat_name_under}_id_list.append(${cat_name_under}.ident)

        request.cls.${pkg_name_replaced}_mgr.add_root_${cat_name_under}(request.cls.${cat_name_under}_id_list[0])
        request.cls.${pkg_name_replaced}_mgr.add_child_${cat_name_under}(request.cls.${cat_name_under}_id_list[0], request.cls.${cat_name_under}_id_list[1])
        request.cls.${pkg_name_replaced}_mgr.add_child_${cat_name_under}(request.cls.${cat_name_under}_id_list[0], request.cls.${cat_name_under}_id_list[2])
        request.cls.${pkg_name_replaced}_mgr.add_child_${cat_name_under}(request.cls.${cat_name_under}_id_list[1], request.cls.${cat_name_under}_id_list[3])
...
class Test${object_name}AuthzAdapter(object):
    ...
    def test_lookup_${cat_name_under}_0_plenary_isolated(self):
        if not is_never_authz(self.service_config):
            janes_${pkg_name_replaced}_mgr = Runtime().get_service_manager(
                '${pkg_name_upper}',
                proxy=JANE_PROXY,
                implementation='TEST_SERVICE_JSON_AUTHZ')
            ${cat_name_under} = janes_${pkg_name_replaced}_mgr.get_${cat_name_under}(self.${cat_name_under}_id_list[0])
    ...
    def test_lookup_${cat_name_under}_1_plenary_isolated(self):
        if not is_never_authz(self.service_config):
            janes_${pkg_name_replaced}_mgr = Runtime().get_service_manager(
                ...
                proxy=JANE_PROXY,
                implementation='TEST_SERVICE_JSON_AUTHZ')
            ${cat_name_under} = janes_${pkg_name_replaced}_mgr.get_${cat_name_under}(self.${cat_name_under}_id_list[1])
            ${cat_name_under}.use_isolated_${cat_name_under}_view()
            ${cat_name_under}.use_plenary_${object_name_under}_view()
    ...
    def test_lookup_${cat_name_under}_2_plenary_isolated(self):
        if not is_never_authz(self.service_config):
            janes_${pkg_name_replaced}_mgr = Runtime().get_service_manager(
                ...
                proxy=JANE_PROXY,
                implementation='TEST_SERVICE_JSON_AUTHZ')
            ${cat_name_under} = janes_${pkg_name_replaced}_mgr.get_${cat_name_under}(self.${cat_name_under}_id_list[2])
            ${cat_name_under}.use_isolated_${cat_name_under}_view()
            ${cat_name_under}.use_plenary_${object_name_under}_view()
            assert ${cat_name_under}.get_${object_name_under_plural}().available() == 1
            assert ${cat_name_under}.get_${object_name_under_plural}_by_genus_type(BLUE_TYPE).available() == 1
    ...
    def test_lookup_${cat_name_under}_3_plenary_isolated(self):
        if not is_never_authz(self.service_config):
            janes_${pkg_name_replaced}_mgr = Runtime().get_service_manager(
                ...
                proxy=JANE_PROXY,
                implementation='TEST_SERVICE_JSON_AUTHZ')
            ${cat_name_under} = janes_${pkg_name_replaced}_mgr.get_${cat_name_under}(self.${cat_name_under}_id_list[3])
            ${cat_name_under}.use_isolated_${cat_name_under}_view()
            ${cat_name_under}.use_plenary_${object_name_under}_view()
            assert ${cat_name_under}.get_${object_name_under_plural}().available() == 3
            assert ${cat_name_under}.get_${object_name_under_plural}_by_genus_type(BLUE_TYPE).available() == 1
