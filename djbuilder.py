...
from djbuilder_settings import ABCROOTPACKAGE as abc_root_pkg
...
def make_persistance_initers(persisted_data, initialized_data):
    initers = ''
    for data_name in persisted_data:
        if (persisted_data[data_name] == 'osid.id.Id' and
                data_name in initialized_data):
            initers = initers + (
                '        ' + data_name + '_id = kwargs[\'' + data_name + '_id\']\n'
                '        self.my_model.' + data_name + '_authority = ' + data_name + '_id.get_authority()\n' +
                '        self.my_model.' + data_name + '_namespace = ' + data_name + '_id.get_identifier_namespace()\n' +
                '        self.my_model.' + data_name + '_identifier = ' + data_name + '_id.get_identifier()\n')
    return initers
    ...
    for data_name in persisted_data:
        ...
        if persisted_data[data_name] == 'boolean':
            template = string.Template(options.COMMON_METADATA +
                                       options.BOOLEAN_METADATA)
            default = default + ('        self._' + data_name +
                                 '_default = self.my_model.options[\'' + data_name + '\'][\'default\']\n')
            ...
            template = string.Template(options.COMMON_METADATA +
                                       options.ID_METADATA)
            default = default + (
                '        self._' + data_name + '_default = dict()\n'
                '        self._' + data_name +
                '_default[\'authority\'] = self.my_model.options[\'' + data_name + '_authority\'][\'default\']\n' +
                '        self._' + data_name +
                '_default[\'namespace\'] = self.my_model.options[\'' + data_name + '_namespace\'][\'default\']\n' +
                '        self._' + data_name +
                '_default[\'identifier\'] = self.my_model.options[\'' + data_name + '_identifier\'][\'default\']\n')
        elif persisted_data[data_name] == 'osid.id.Id[]':
            pass  # Put id-table style initters here
        elif persisted_data[data_name] == 'osid.mapping.SpatialUnit':
            pass  # Put SpatialUnit initters here

        if template:
            initer = (initer + '        self._' + data_name + '_metadata = {' +
...
def make_profile_py(package):
    ...
    try:
        old_profile = import_module('.'.join(app_name(package['name']).split('/')) + '.' +
