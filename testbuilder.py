...
def make_tests(build_abc = False, re_index = False, re_map = False):
    ...
    for json_file in os.listdir(pkg_maps_dir):
        if json_file.endswith('.json'):
            make_package_tests(pkg_maps_dir + '/' + json_file)
    ##
...
def make_package_tests(file_name):
    """
    ...
    if package['name'] not in packages_to_test:
        return
    print "--> Test Package:", package['name']
    ...
    for interface in package['interfaces']:
        ...
        ##
        # Just to get started, we will implement one category at a time
        if (interface['category'] != 'managers' and
        ...
        last_inheritance = []
        ##
        # Seed the inheritance list with unittest.TestCase
        inheritance = ['unittest.TestCase']
        ##
        # And make sure there is a corresponding import statement
        ...
        #     class_doc = ('    \"\"\"' + interface['doc']['headline'] + '\n\n' +
        #                             interface['doc']['body'] + '\n\n    \"\"\"')
        class_doc = '    \"\"\"Tests for ' + interface['shortname'] + '\"\"\"'
            
        class_sig = 'class Test' + interface['shortname'] + inheritance + ':'
...
#        args.append(arg['var_name'] + '=None')
    method_sig = ('    def test_' + method['name'] + '(' +
                ', '.join(args) + '):')
    
    if method_impl == '        pass':
        method_sig = '    @unittest.skip(\'unimplemented test\')\n' + method_sig

    method_doc = '        \"\"\"Tests '+ method['name'] + '\"\"\"'
...
def make_method_impl(package_name, method, interface, patterns):
    ...
    # method implementation that serves as the pattern, if one exixts.
        ...
        if is_manager_session(interface, patterns, package_name):
            kwargs['svc_mgr_or_catalog'] = 'svc_mgr'
        else:
            kwargs['svc_mgr_or_catalog'] = 'catalog'
